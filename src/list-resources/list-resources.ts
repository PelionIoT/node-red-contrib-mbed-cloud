...
class ListResources {
    ...
    private inputHandler(msg) {
        ...
        this.connect.listResources(deviceId)
            .then(resources => {
                ...
                this.node.status({
                    fill: "red",
                    shape: "ring",
                    text: "Error retreiving resources"
                });
...
export = RED => {
    ...
    RED.nodes.registerType("list-resources", function(config) {
        const node = this;
        RED.nodes.createNode(node, config);
        // tslint:disable-next-line:no-unused-expression
        new ListResources(node, config, RED);
    });
};
