...
class StartCampaign {
    ...
    private inputHandler(msg) {
        ...
        this.update.startCampaign(campaignId)
            .then(data => {
...
export = RED => {
    ...
    RED.nodes.registerType("start-campaign", function(config) {
        const node = this;
        RED.nodes.createNode(node, config);
        // tslint:disable-next-line:no-unused-expression
        new StartCampaign(node, config, RED);
    });
};
