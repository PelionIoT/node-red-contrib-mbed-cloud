...
class ExecuteResource {
    ...
    private inputHandler(msg) {
        ...
        this.connect.executeResource(deviceId, resourceURI)
            .then(data => {
...
export = RED => {
    ...
    RED.nodes.registerType("execute-resource", function(config) {
        const node = this;
        RED.nodes.createNode(node, config);
        // tslint:disable-next-line:no-unused-expression
        new ExecuteResource(node, config, RED);
    });
};
